...
6. Dans Batlay : **Connections > Spotify**, collez le Client ID, puis cliquez sur **Connect Spotify**.
...
Pour les comptes gratuits, **Connections > Lecture système (Windows) > Connect** lit directement les métadonnées "En cours de lecture" exposées par Windows via les *Global System Media Transport Controls* (la même API que le Volet de contrôle multimédia / les touches multimédias du clavier) :
...
- **Créer** : « Create Overlay » propose **Partir de zéro** (canvas vide, nom + taille) ou un des 6 presets (avec miniatures). Les presets existants sont inchangés.
- **Composants** (bouton *Ajouter un composant*) : pochette, titre, artiste(s), album, barre de progression, temps écoulé, **temps restant**, durée, source, **texte libre** (variables `{title} {artist} {album} {source}`), **forme / fond** (carte, séparateur…).
...
- **Raccourcis** : `Ctrl+S` enregistrer · `Ctrl+Z` / `Ctrl+Y` · `Ctrl+D` dupliquer · `Suppr` · flèches = 1 px (`Maj` = 10 px) · `Ctrl+[` / `Ctrl+]` calque en dessous / au-dessus · pendant un glissement : `Maj` = axe droit ou proportions, `Alt` = sans magnétisme ou depuis le centre · `Échap` désélectionne.
...
1. Dans Batlay, ouvrez un overlay dans l'**Editor**.
2. Copiez l'**Overlay URL** affichée en bas du panneau Properties (ex. `http://localhost:3000/overlay/abc123`).
...
Chaque overlay peut être exporté en `.json` (`schemaVersion: 1`) depuis la page **My Overlays**. L'import avec validation stricte du schéma **n'est pas encore implémenté** — actuellement seul l'export fonctionne.
