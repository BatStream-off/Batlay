...
export function LayersPanel(props: LayersPanelProps) {
  ...
  // Palette ouverte d'office sur un overlay vide (le premier geste évident), puis
  // repliable : elle reste ouverte après un ajout pour en enchaîner plusieurs.
  const [paletteOpen, setPaletteOpen] = useState(components.length === 0);
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-line p-3">
        ...
        >
          <Plus size={14} className={`transition ${paletteOpen ? "rotate-45" : ""}`} />
          {paletteOpen ? "Fermer la palette" : "Ajouter un composant"}
        </button>
        {paletteOpen && (
            ...
            {COMPONENT_TYPES.map((type) => {
              ...
              return (
                <button
                  ...
                  className="flex flex-col items-center gap-1 rounded-lg border border-base-700 bg-base-800 px-1 py-2 text-center text-[11px] leading-tight text-muted transition hover:border-signal-500 hover:text-fg"
                >
      ...
      <div className="flex-1 overflow-y-auto p-2" onDragLeave={() => setOverIndex(null)}>
        {rows.length === 0 ? (
          <p className="px-2 py-6 text-center text-xs text-muted">
            Overlay vide. Cliquez sur un composant de la palette pour l'ajouter.
          </p>
        ) : (
          rows.map((c, rowIndex) => {
