...
import { CANVAS_SIZES, COMPONENT_LABELS, type AlignKind } from "@/utils/overlay-model";
...
type PropertiesTab = "general" | "style" | "appearance";

export function PropertiesPanel(props: PropertiesPanelProps) {
  const { selected } = props;
  // L'onglet est conservé quand on change de composant : régler la couleur de
  // dix textes à la suite ne doit pas obliger à rouvrir « Apparence » à chaque fois.
  const [tab, setTab] = useState<PropertiesTab>("general");
  return (
    <div className="px-4 pb-6">
      {selected ? (
        <ComponentProperties {...props} component={selected} tab={tab} onTab={setTab} />
      ) : (
...
function ComponentProperties(
  props: PropertiesPanelProps & { component: OverlayComponentConfig; tab: PropertiesTab; onTab: (tab: PropertiesTab) => void }
) {
  const { component: c, onPatchComponent, onPatchTransform, onPatchStyle, onAlign, tab, onTab } = props;
  const s = c.style;
  ...
  // « Style » = réglages propres au type (texte, pochette, barre). Un simple
  // rectangle n'en a pas : l'onglet disparaît plutôt que d'afficher du vide.
  const hasStyleTab = isText || c.type === "artwork" || c.type === "progressBar";
  const styleTabLabel = isText ? "Texte" : c.type === "artwork" ? "Pochette" : "Barre";
  const tabs: { id: PropertiesTab; label: string }[] = [
    { id: "general", label: "Général" },
    ...(hasStyleTab ? [{ id: "style" as const, label: styleTabLabel }] : []),
    { id: "appearance", label: "Apparence" },
  ];
  // Onglet mémorisé devenu inexistant (ex. « Texte » puis sélection d'une forme) : retour à « Général ».
  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-10 -mx-4 border-b border-line bg-base-950 px-4 pb-2 pt-3">
        ...
        />
        <div role="group" aria-label="Sections des propriétés" className="mt-3">
          <SegmentedField value={activeTab} onChange={onTab} options={tabs.map((t) => ({ label: t.label, value: t.id }))} />
        </div>
      </div>
      ...
      <Section title="Position et taille">
        ...
        <p className="text-[10px] text-faint">Tous les raccourcis : bouton clavier au-dessus de l'aperçu.</p>
      </Section>
        </>
      )}
      ...
      {obsUrl && (
        <div className="mt-2 border-t border-line pt-4">
          ...
          <p className="mt-2 text-[11px] text-muted">
            Taille de la source dans OBS : {t.canvasWidth} × {t.canvasHeight}
          </p>
