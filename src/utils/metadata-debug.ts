 ...
 *   - "fenetre-spotify": le titre de la fenêtre Spotify ne correspond pas à
 *                        « artiste - titre » de SMTC (pub, changement de piste,
 *                        artiste incomplet...) : c'est le cas à envoyer pour diagnostic.
 */

export const METADATA_DEBUG_STORAGE_KEY = "batlay:debug-metadata";

export type MetadataDebugCase = "accent" | "multi-artistes" | "non-latin" | "fenetre-spotify";
...
export interface MetadataFields {
  ...
  albumArtist?: string;
  /** Titre de la fenêtre Spotify Desktop, quand il est lisible (voir spotify-metadata.ts). */
  windowTitle?: string;
}
...
export function classifyMetadataDebugCases(fields: MetadataFields): MetadataDebugCase[] {
  ...
  if (fields.windowTitle && fields.windowTitle !== `${fields.artist} - ${fields.title}`) {
    cases.push("fenetre-spotify");
  }
  return cases;
}
  ...
  return function logMetadata(trackKey: string, raw: MetadataFields, cleaned: MetadataFields): void {
    ...
    write(
      [
        ...
        ...(raw.windowTitle ? [formatField("fenêtre Spotify", raw.windowTitle, undefined)] : []),
      ].join("\n")
    );
  };
}
