...
export function isPlaceholderArtist(artist) {
    const v = tidy(artist).toLowerCase().replace(/\.(exe|app)$/, "");
    if (!v)
        return true;
    if (PLACEHOLDER_ARTISTS.has(v))
        return true;
 ...
 */
const NOISE_PATTERN = /\b(official|officiel|officielle|video|vidéo|audio|lyrics?|paroles|visualizer|visualiser|mv|hd|hq|sd|4k|8k|1080p|720p|remaster(ed)?|remasterisé|explicit|clean|clip|music\s*video|full\s*album|color\s*coded|legendado|topic)\b/i;
/**
...
function stripNoiseBrackets(value) {
    return value
        .replace(/[([{]([^)\]}]*)[)\]}]/g, (match, inner) => NOISE_PATTERN.test(inner) ? " " : match)
        .replace(/\s{2,}/g, " ")
        .trim();
}
...
export function cleanTitle(title) {
    return stripNoiseBrackets(tidy(title)
 ...
 */
export function normalizeMetadata(rawArtist, rawTitle) {
 ...
 */
export function buildQueryCandidates(meta) {
    const candidates = [];
    const seen = new Set();
 ...
 */
export function isPlausibleMatch(candidate, match) {
    const titleOk = similarity(candidate.title, match.title) >= TITLE_SIMILARITY_THRESHOLD ||
        looseIncludes(candidate.title, match.title);
    if (!titleOk)
        return false;
    // Pas d'artiste des deux côtés : le titre seul décide (seuil déjà appliqué).
    if (!candidate.artist || !match.artist)
        return true;
    return (similarity(candidate.artist, match.artist) >= ARTIST_SIMILARITY_THRESHOLD ||
...
export function upscaleItunesArtwork(url, size = 600) {
    return url.replace(/\/\d+x\d+bb\.(jpg|png)$/i, `/${size}x${size}bb.$1`);
}
async function searchItunes(candidate) {
    if (!itunesRateLimitOk())
    ...
    for (const item of results) {
        ...
        if (isPlausibleMatch(candidate, match))
            return { match, failed: false };
    }
    return { match: null, failed: false };
}
async function searchDeezer(candidate) {
    const since = Date.now() - lastDeezerCall;
    if (since < DEEZER_MIN_INTERVAL_MS)
        await sleep(DEEZER_MIN_INTERVAL_MS - since);
    ...
    if (!outcome.ok)
        return { match: null, failed: true };
    const results = outcome.data?.data ?? [];
    for (const item of results) {
 ...
 */
async function searchCoverArtArchive(candidate) {
    const since = Date.now() - lastMusicBrainzCall;
    if (since < MUSICBRAINZ_MIN_INTERVAL_MS)
        await sleep(MUSICBRAINZ_MIN_INTERVAL_MS - since);
    lastMusicBrainzCall = Date.now();
    const escape = (v) => v.replace(/["\\]/g, " ").trim();
    const clauses = [`recording:"${escape(candidate.title)}"`];
    if (candidate.artist)
        clauses.push(`artist:"${escape(candidate.artist)}"`);
    const url = `${MUSICBRAINZ_ENDPOINT}?query=${encodeURIComponent(clauses.join(" AND "))}&limit=5&fmt=json`;
    const outcome = await fetchJson(url);
    if (!outcome.ok)
        return { match: null, failed: true };
    const recordings = outcome.data?.recordings ?? [];
    for (const rec of recordings) {
        const artist = (rec["artist-credit"] ?? [])
            .map((c) => c.name ?? "")
            .join(" ")
            .trim();
        const probe = { url: "", artist, title: rec.title ?? "" };
        if (!isPlausibleMatch(candidate, probe))
            continue;
        ...
        try {
            ...
            if (head.ok)
                return { match: { ...probe, url: coverUrl }, failed: false };
        }
        catch {
            return { match: null, failed: true };
        }
 ...
 */
async function trySource(source, candidates, deadline) {
    for (const candidate of candidates) {
        if (Date.now() >= deadline)
            return { url: null, source: "none", failed: true };
        const attempt = await source.search(candidate);
        if (attempt.match)
            return { url: attempt.match.url, source: source.id, failed: false };
...
async function resolve(rawArtist, rawTitle) {
    const meta = normalizeMetadata(rawArtist, rawTitle);
    if (!meta.title)
        return { url: null, source: "none", definitive: true };
    ...
    const winner = await Promise.race([
        itunesRun,
        sleep(HEDGE_DELAY_MS).then(() => null),
    ]);
    if (winner?.url)
        return { url: winner.url, source: winner.source, definitive: true };
    ...
    for (const result of [itunesResult, deezerResult]) {
        if (result.url)
            return { url: result.url, source: result.source, definitive: true };
    }
 ...
 */
export async function lookupArtwork(artist, title) {
    try {
...
export function __clearArtworkCache() {
    cache.clear();
    inFlight.clear();
...
//# sourceMappingURL=artwork-lookup.js.map